...
#ifdef CONFIG_M2_V9X5_DISPLAY_104
...
static struct fb_videomode at91_tft_vga_modes[] = {
	{
		...
		.left_margin	= 80,		.right_margin	= 40,
		.upper_margin	= 21,		.lower_margin	= 21,
...
static struct i2c_board_info i2c3_devs[] __initdata = {
	{
		...
		.irq		= gpio_to_irq(AT91_PIN_PD16),
	},
};


static void __init vulcano9x5_tsp_init(void)
{
	printk("Inizialization vulcano9x5_tsp_init.\n");
	/* If enabled, give a reset impulse */
	...
	if(at91_set_gpio_input(AT91_PIN_PD16, 1)) {
	    printk(KERN_DEBUG"Could not set pin %i for GPIO input.\n", AT91_PIN_PD16);
	}

	/** Set deglitch for pin */
	if(at91_set_deglitch(AT91_PIN_PD16, 1)) {
...
static void __init ek_board_init(void)
{
	...
	if (!(cm_config & CM_CONFIG_SPI0_ENABLE))
		...
		printk("Cannot enable wifi due to SPI or ISI configuration\n");
	...
	#ifdef CONFIG_M2_V9X5_DISPLAY_104
		vulcano9x5_tsp_init();
		i2c_register_board_info(0, i2c3_devs, ARRAY_SIZE(i2c3_devs));
	#else
